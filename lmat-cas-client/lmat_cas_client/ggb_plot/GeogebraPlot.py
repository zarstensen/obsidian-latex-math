...
class GeogebraPlot:
    ...
    def plot_function(
        self, name: str, args: Iterable[Symbol], expr: Expr, visible: bool = True
    ) -> tuple[Element, tuple[Element]]:
        constants = self._define_constants(expr.free_symbols - set(args))

        expression_element = ElementTree.SubElement(
            self._construction,
            "expression",
            label=name,
            exp=f"{name}({','.join((str(a) for a in args))}) = {print_geogebra(expr)}",
        )

        if not visible:
            ElementTree.SubElement(
                self._construction, "element", type="function", label=name
            ).extend([Element("show", object="false")])
    ...
    def plot_relation(self, sympy_relation: Relational) -> Element:
        ...
        sympy_relation = type(sympy_relation)(
            self._try_replace_func(lhs), self._try_replace_func(rhs)
        )
    ...
    def plot_contour_curves(self, expr: Expr):
        ...
        ElementTree.SubElement(
            ...
            "expression",
            label="CenterPoint",
            exp="Corner[4] - (Corner[4] - Corner[2]) / 2",
            type="point",
        )
        ...
        with zipfile.ZipFile(out_file, "w", zipfile.ZIP_DEFLATED) as zipf:
            zipf.writestr(
                ...
                ElementTree.tostring(
                    self._ggb_template_file.getroot(), encoding="utf8"
                ),
            )
