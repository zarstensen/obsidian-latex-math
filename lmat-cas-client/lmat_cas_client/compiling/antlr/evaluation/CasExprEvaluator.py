...
from typing import Iterable, NamedTuple, NamedTuple, cast, override
...
def evalExprToSympy(expr: Ast.Expr, scope: None) -> CasExprV2:
	...
	match expr:
		case Ast.AstNode(ctx):
			return tuple([(ev(expr, scope), cast(tuple[int, int], ctx.getSourceInterval()))])
